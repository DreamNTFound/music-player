...
export default function Sidebar({ setlibraryItems }) {
  ...
  return (
    <aside
      className={`relative h-screen flex-shrink-0 bg-black text-white p-4 transition-all duration-300 ${
        collapsed ? "w-20" : "w-64"
      }`}
    >
      {/* Toggle */}
                ...
                if (item.isCreate) {
                  return (
                    <button
                      ...
                      className="flex items-center gap-3 !px-3 !py-2 rounded-md transition text-white hover:bg-purple-700/30 hover:scale-105"
                    >
                      <FontAwesomeIcon
                        icon={item.icon}
